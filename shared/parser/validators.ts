...
const isArray = (x: any, path: string) => {
    if (!Array.isArray(x)) {
        throw Error(
            `value at "${path}" is not an array even though an array is expected`,
        );
    }
};
...
export function cUrlParser(obj: object, path: string): cURL {
    ...
    if (value.url == undefined) {
        throw Error(`missing ".url" for cURL entry at "${path}"`);
    }
    if (value.type == undefined) {
        throw Error(`missing ".type" for cURL entry at "${path}"`);
    }
...
export function cUrlArrayParser(obj: object, path: string): cURL[] {
    const value = eval(`obj${path}`);
    isArray(value, path);
    return (value as any[]).map((_, index) =>
        cUrlParser(obj, `${path}[${index}]`)
    );
}
...
export function isPlatformObj(
    obj: object,
    path: string,
): { [key in Platform]?: Version | true } {
    const value = eval(`obj${path}`);
    if (value == undefined) {
        throw Error(`Platforms Object required but missing at "${path}"`);
    }
...
export function releaseArrayParser(obj: object, path: string): Release[] {
    ...
    isArray(value, path);
    return (value as any[]).map((_, index) =>
        releaseParser(obj, `${path}[${index}]`)
    );
}
export function isLanguageArray(obj: object, path: string): Language[] {
    const value = eval(`obj${path}`);
    isArray(value, path);
    return (value as any[]).map((v, index) =>
        isStringsInEnum(v, Language, `${path}[${index}]`)
    ) as Language[];
}
...
export function isTandemManifest(
    obj: object,
    path: string,
): TandemManifest | undefined {
    const value = eval(`obj${path}`);
    if (value == undefined) return undefined;
    if (typeof value != "object") {
        throw Error(
            `optional TandemManifest at "${path}" needs to be undefined or object`,
        );
    }
...
export function isTypes(obj: object, types: valueTypes[], path: string) {
    ...
    throw Error(
        `"${value}" at "${path}" is not any of these types "${
            types.join(", ")
        }"`,
    );
}

export const isNumber = (obj: object, path: string) =>
    isType(obj, "number", path);
export const isString = (
    obj: object,
    path: string,
    optional: boolean = false,
) => isType(obj, "string", path, optional);
export const isBoolean = (obj: object, path: string) =>
    isType(obj, "boolean", path);
export const isStringOrBool = (obj: object, path: string) =>
    isTypes(obj, ["string", "boolean"], path);
/** options = the enum */
export function isStringsInEnum(str: string, options: object, path: string) {
    if (!(Object.values(options).includes(str))) {
        throw Error(
            `"${str}" at "${path}" is not a valid value for this field. Allowed values are: "${
...
export function isCapabilityObj(
    obj: object,
    path: string,
    _enum: object,
    ...
    const value = eval(`obj${path}`);
    if (value == undefined) {
        throw Error(`Capability Object required but missing at "${path}"`);
    }
    Object.entries(value).forEach(([key, value]) => {
